    ...
    if (campaign_data) {
        ...
        if (user && proxy && adsets.length) {
            ...
            for (let i = 0; i < adsets.length; i++) {
                console.log("adset creating i: " + i)
                let data = await fb.ad_set.create(user, proxy, { ad_account_id: body.ad_account_id }, adsets[i].adset_fb)
                if (data) {
                    if (data.id) {
                        if (adsets[i].adset_doc.ads) {
                            for (let y = 0; y < adsets[i].adset_doc.ads.length; y++) {
                                console.log("adset creating i: " + i + " / ad: " + y)
                                let ad = adsets[i].adset_doc.ads[y]
...
router.post("/create/v2", async (req, res) => {
    ...
    if (campaign_data) {
        ...
        if (user && proxy && adsets.length) {
            ...
            for (let i = 0; i < adsets.length; i++) {
                if (adsets[i].adset_doc.ads && adsets[i].fb_id) {
                    for (let y = 0; y < adsets[i].adset_doc.ads.length; y++) {
                        console.log("adset creating i: " + i + " / ad: " + y)
                        let ad = adsets[i].adset_doc.ads[y]
                        let image_doc = await db.Images.get_by_id(ad.file_image)
